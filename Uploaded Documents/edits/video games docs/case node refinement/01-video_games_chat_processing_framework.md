...
Example:
> 1. Game-Like Digital Component Subordinate to a Real-World Activity — direct match.
...
Example:
> 2. Game-Like Digital Component Subordinate to a Real-World Activity — partial match. The subordinate-component logic is relevant, but the task evidence is mixed about whether the digital experience or the real-world activity is primary.

### Broad conceptual match
Use when the node does not directly control the rating, but it helps frame the distinction.

Example:
> 3. Game-Like Digital Component Subordinate to a Real-World Activity — broad conceptual match. It helps distinguish a game-like secondary feature from the main advertised product.
...
Assign each applicable node a role where the distinction is meaningful:

- **Primary** — provides the main reasoning structure.
- **Supporting** — contributes a distinct secondary rule, evidence treatment, or boundary.
- **Contrast** — helps explain why a nearby or superficially similar pattern does not control the rating.
...
7. Identify the **most abstract but still maximally useful frame** for the reasoning node: generalize beyond incidental names and details, but stop before the abstraction loses the decisive mechanism, evidence relationship, or boundary that makes the precedent useful. Test every proposed title, case type, decision shortcut, and use condition by asking whether the same reasoning would still apply without each included feature. If yes, keep that feature as anchor evidence rather than a node-level eligibility condition.
8. Review and rank all existing case nodes that materially apply. Decide whether the case should extend an existing node, add a specialized supporting node, synthesize existing nodes, or justify a genuinely new node. Avoid near-duplicate nodes at slightly different abstraction levels.
9. Extract abandoned interpretations and wording traps, then define use and do-not-use boundaries.
10. Review and rank all existing comment patterns that materially apply. Reuse or extend existing wording where possible, and extract only genuinely new reusable wording at the most useful level: complete template, sentence, clause, phrase structure, or individual phrase. Do not force a separate abstract comment “principle.”
11. Create Output A for the Video Games manual and Output B for the rating comment guide. Output A may extend, specialize, or synthesize existing manual material; Output B may add to an existing pattern rather than create a new whole pattern.
...
- Extract visible evidence separately from inference.
- Research every visible clue that could be an entity, title, brand, slogan, acronym, specialist term, or ordinary-looking phrase with a relevant secondary meaning.
- Search exact wording and useful contextual variants, prefer authoritative primary sources, disambiguate ambiguous or same-named identities, and preserve material resolved and unresolved findings.
- Flag any verified research that materially conflicts with the verdict anchor before producing reusable guidance.
- Identify the most abstract but still maximally useful reasoning frame. Make node titles, case types, shortcuts, and use conditions describe the decisive mechanism rather than incidental characteristics of the anchor example.
- Review and rank all existing case nodes and comment patterns that materially apply. Extend, specialize, or synthesize existing material where appropriate rather than creating a duplicate, and extract only genuinely new reusable wording.
...
- Do not manufacture a separate abstract “comment principle.”

Ground the outputs in the visible evidence and verified research tied to it. Do not treat researched identification of something visibly present as hidden landing-page behavior. Do not preserve unsupported assumptions about unseen post-click content. Preserve useful uncertainty and negative direction.
```
...
- Did I identify the most abstract but still maximally useful reasoning frame?
- Do the node title, case type, decision shortcut, and use conditions encode the decisive mechanism rather than incidental anchor details?
- If the same reasoning would apply without a listed feature, did I move that feature to anchor evidence instead of making it a node-level condition?
- Did I review and rank all materially applicable existing nodes before deciding whether to extend, specialize, synthesize, or create?
- Did I review and rank all materially applicable existing comment patterns and extract only genuinely new reusable wording?
