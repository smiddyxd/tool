...
### Rating Notes Library — targeted second-guess guardrails

These notes are not standalone rating rules and do not replace the standard machinery. They exist to prevent repeated directional mistakes that arise when the analysis converges too quickly on one plausible reading, one requirement-weighting choice, or one type of relatedness.

They should be consulted only after the standard machinery has produced a provisional rating. If a note materially applies, the analysis must briefly surface:
1. why the note is relevant,
2. what earlier rating premise it challenges,
3. whether the provisional rating should stay or be revised.

The purpose is not to manufacture uncertainty. The purpose is to consciously test known failure patterns before treating the provisional rating as settled.

#### Rating Note 1 — ambiguous or weird queries may require Google search-results research, not just term lookup
...
- the analysis would materially change depending on which interpretation is dominant.

Guardrail:
- Research the **Google search-results landscape** for the query itself to assess the plausible meaning space.
- Do not let the shown product create the operating interpretation merely because it can be made to fit.
- If search results surface several concrete interpretations that are unrelated to the shown product, and the product only survives through superficial word/title overlap, that strongly pressures the rating downward.
- A weird query should not be casually reinterpreted into a product-friendly meaning when search results suggest otherwise.

Typical failure prevented:
- over-crediting a word-match product for a genuinely ambiguous or odd query.

#### Rating Note 2 — explicit brand / retailer / product-line mentions are only as important as the real constraint they imply

Use this note when the provisional rating is pushed downward mainly because the product misses:
- a brand,
- a retailer,
- a manufacturer,
- or a product line.
...
Guardrail:
- Do not treat the mere fact that a brand or retailer was explicitly named as inherently high-weight.
- State what practical implication the commercial anchor carries here:
  - compatibility,
  - officialness,
  - prestige/tier,
...
- If no strong practical implication exists and the substitute preserves the product category, function, and approximate shopping role, the brand/retailer miss should be lighter.
- Do not stack brand + manufacturer + retailer as separate harsh penalties when they express one shared commercial idea.

Typical failure prevented:
- pushing decent substitutes into Poor merely because the query names a brand or retailer.

#### Rating Note 3 — nearby intent / same problem space can be Okay even when direct substitution fails

Use this note when:
- the product is not a direct substitute,
- but it materially participates in solving the same practical problem,
- or it plausibly advances the same user goal through a nearby route.

Guardrail:
- Re-check whether the task fits **same problem space / relevant product** rather than **bad substitute**.
- The product does not need to be interchangeable with the queried item to reach Okay if it is genuinely useful in the same problem context.
- Do not downgrade to Poor simply because the product serves the adjacent phase of the same job rather than the exact requested phase.

Typical failure prevented:
- undervaluing same-problem-space products that guidelines treat as relevant.

#### Rating Note 4 — informational queries can be helped by products that materially enable the user’s goal

Use this note when:
- the query is informational,
- and the product is not an answer,
- but it is a concrete tool, material, or product that substantively helps the user perform the queried activity or achieve the queried end state.

Guardrail:
- For instructional or commercial-in-spirit information queries, consciously distinguish:
  - merely topic-related products,
  - versus products that materially help accomplish the activity.
- A product that directly enables the queried activity can be Okay under **Information Seeking – Result is a relevant product**, even though it does not provide instructions.

Typical failure prevented:
- over-penalizing “how to” queries when the product is genuinely activity-enabling.

#### Rating Note 5 — broad idea-seeking information queries should not over-credit niche product suggestions

Use this note when:
- the query asks for ideas, recommendations, or broad suggestions,
- and the product is only one narrow branch of the idea space.
...
- A product may still be relevant, but its rating should fall when it is unusually narrow across several independent dimensions:
  ...
  - occasion fit,
  - premium pricing,
  - unusually distinctive taste profile,
  - or a very particular use case.
- Do not treat “it could be one possible idea” as enough for mid/high Okay if the product is too niche relative to the broad query population.

Typical failure prevented:
- overrating a narrow product simply because the query is open-ended.

#### Rating Note 6 — inherently non-commercial factual queries are not rescued by same-name merchandise
...
Typical failure prevented:
- incorrectly lifting obvious non-commercial-query mismatches into Poor through “exact-anchor” logic.

#### Rating Note 7 — style-led / aesthetic queries require real style comparison, not shallow category overlap

Use this note when:
- style, visual language, color, finish, print, pattern, or design identity is load-bearing;
- especially in fashion, decor, and other mechanism-divergent or self-expression-heavy domains.

Guardrail:
- Distinguish a true style match from a visually adjacent but meaningfully different style.
- Aesthetic similarity can support a substitute rating, but it should not erase meaningful differences in the named style mechanism.
- Named color or visual-identity misses can matter substantially when they are the query’s main differentiator or part of a recognizable model identity.

Typical failure prevented:
- treating meaningful style/color differences as trivial, or conversely collapsing adjacent aesthetics into a near-match.

#### Rating Note 8 — check non-obvious requirement fulfillment before declaring a miss

Use this note when:
- the provisional rating hinges on a queried property that may be visible in the image, strongly implied by the visible product form, or easy to miss if one looks only at title text.

Guardrail:
- Deliberately inspect visible evidence before classifying the requirement as missed.
- If the property appears visually present, credit it.
- If it remains unclear, mark it unclear or only partly supported rather than converting uncertainty into a hard miss.
- Do not let “not spelled out in the title” alone sink a product that is otherwise very close.

Typical failure prevented:
- under-crediting products because a requirement is non-obvious or visually shown rather than textually repeated.

#### Rating Note 9 — a specific product can satisfy a generic query without becoming “too niche” by default

Use this note when:
- the query is generic or broad,
- and the shown product is a concrete SKU with normal attributes like color, age band, small capacity range, training wheels, common variant styling, or ordinary retailer-specific packaging.

Guardrail:
- Every real product is specific. Do not penalize specificity merely because the query was broad.
- Ask whether the product’s extra specifics are:
...
Typical failure prevented:
- rating a sensible broad-query match too low because the shown SKU has normal real-world details.

#### Rating Note 10 — price differences matter only when they imply a real tier mismatch
...
Typical failure prevented:
- over-penalizing viable substitutes for price differences that do not materially change the shopping role.
...
Every top-level section in Part 2 is mandatory and must appear in the final output, in the same order as written.
...
16. Position Calibration Check
17. Borderline cases
18. Categorical Miss Subtype Assessment
19. Override Impact
20. Formatting compliance check
...
**Rating:** [tier] ([low / mid / high])
**Position rationale:**
- [bullet citing a specific Rating Synthesis indicator reading or Part 1 position mechanism — positive justification for the tier and position, not restated elimination reasons  **Each cited mechanism must be scoped to within-band positioning, not to band selection or floor/ceiling boundaries.**]
- [add bullets as needed]

## Position Calibration Check (on the Standard-Machinery Rating)

Operates on the rating produced by Standard-Machinery Rating Suggestion above.
...
Sits at the end of the analysis, after the Standard-Machinery Rating Suggestion and Position Calibration Check are complete. Those produce the rating under default operating mode. This section evaluates whether a categorical-miss subtype applies as an override on top of that already-locked rating.
